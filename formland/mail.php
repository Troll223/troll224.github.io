...
const NOTIFICATIONS_EMAIL = "tolyaimag@gmail.com";
...
function sendTelegram($input, $leadId, $errorFiles, $goodFiles)
{
    ...
    $message = $input["text"]["leadId"]
        ? "Апселл к заказу {$input["text"]["leadId"]} с сайта $siteName\n\n"
        : "Новый заказ на сайте $siteName\n\nID: $leadId\n";
    foreach ($input["text"] as $name => $value) {
...
function showDefaultThankyouPage($input)
{
    if ($input["text"]["leadId"]) {
        $html = "<h3>Товар добавлен к заказу</h3>"
            . "<p>Мы свяжемся с вами в течение 15 минут для подтверждения заказа. "
            . "Пожалуйста, убедитесь, что ваш телефон включен</p><ul>";
        foreach ($input["text"] as $name => $value) {
            if ($name != "redirect" && $name != "leadId" && $name != "MAX_FILE_SIZE") $html .= "<li>$name: $value</li>";
        }
        $html .= "</ul>";
    } else {
        $html = "<h3>ВАША ЗАЯВКА ПРИНЯТА</h3>"
            . "<p>Мы свяжемся с вами в течение 15 минут для подтверждения заказа. "
            . "Пожалуйста, проверьте введённые данные:</p><ul>";
        foreach ($input["text"] as $name => $value) {
            if ($name != "redirect" && $name != "MAX_FILE_SIZE") $html .= "<li>$name: $value</li>";
        }
        $html .= "</ul><p>Если Вы ошиблись, то закройте это окно, заполните и отправьте форму заказа еще раз</p>";
    }
    if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
        header("Content-Type: application/json; charset=utf-8");
